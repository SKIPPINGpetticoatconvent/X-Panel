...
func updateXray(version string) {
	serverService := service.ServerService{}
	if version == "latest" || version == "" {
		versions, err := serverService.GetXrayVersions()
		if err != nil {
			fmt.Println("Error getting Xray versions:", err)
			return
		}
		if len(versions) == 0 {
			fmt.Println("No Xray versions found.")
			return
		}
		version = versions[0]
	...
	fileName, err := serverService.DownloadXRay(version)
	if err != nil {
		fmt.Println("Error downloading Xray-core:", err)
		return
	}
	defer os.Remove(fileName)

	fmt.Println("Installing Xray-core...")
	err = serverService.InstallXrayBin(fileName)
	if err != nil {
		fmt.Println("Error installing Xray-core:", err)
		return
	}
	fmt.Println("Xray-core updated successfully. Please restart the panel to apply changes.")
}

func GetCertificate(show bool) {
	if show {
		settingService := service.SettingService{}
		certFile, err := settingService.GetCertFile()
		if err != nil {
...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Println("    update         update xray")
	}
	...
	case "update":
		if len(os.Args) < 3 {
			fmt.Println("Usage: x-ui update <target> [version]")
			return
		}
		target := os.Args[2]
		version := ""
