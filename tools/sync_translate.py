...
def parse_tgbot_messages(content):
    ...
    for line in lines:
        ...
        if stripped == '[tgbot.messages]':
            in_section = True
            continue
        if in_section and stripped.startswith('[') and not stripped.startswith('[tgbot.messages]'):
            break
...
def process_file(filepath, master_keys):
    ...
    for line in lines:
        ...
        if in_messages and stripped.startswith('[') and not stripped.startswith('[tgbot.messages]'):
