...
class WsStreamSettings extends CommonClass {
	...
	static fromJson(json = {}) {
		return new WsStreamSettings(json.path, json.host, json.heartbeatPeriod);
	}
...
class Outbound extends CommonClass {
	...
	toJson() {
		...
		return {
			...
			settings:
				this.settings instanceof CommonClass ? this.settings.toJson() : this.settings,
	...
	static fromVmessLink(json = {}) {
		...
		if (network === "tcp") {
			stream.tcp = new TcpStreamSettings(json.type, json.host ?? "", json.path ?? "");
		} else if (network === "kcp") {
		...
		return new Outbound(
			json.ps,
			Protocols.VMess,
			new Outbound.VmessSettings(json.add, port, json.id, json.scy),
			stream,
		);
	}
		...
		switch (protocol) {
			case Protocols.VLESS:
				settings = new Outbound.VLESSSettings(
					...
					url.searchParams.get("encryption") ?? "none",
				);
				break;
			case Protocols.Trojan:
...
Outbound.FreedomSettings = class extends CommonClass {
	constructor(domainStrategy = "", redirect = "", fragment = {}, noises = []) {
		super();
...
Outbound.FreedomSettings.Noise = class extends CommonClass {
	constructor(type = "rand", packet = "10-20", delay = "10-16", applyTo = "ip") {
		super();
		this.type = type;
		this.packet = packet;
		this.delay = delay;
...
Outbound.WireguardSettings = class extends CommonClass {
	...
	toJson() {
		return {
			...
			domainStrategy: WireguardDomainStrategy.includes(this.domainStrategy)
				? this.domainStrategy
				: undefined,
