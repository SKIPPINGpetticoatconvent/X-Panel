...
TlsStreamSettings.Cert = class extends XrayCommonClass {
  constructor(
    ...
    this.cert = Array.isArray(certificate)
      ? certificate.join("\n")
      : certificate;
...
class RealityStreamSettings extends XrayCommonClass {
  constructor(
    ...
    this.xver = xver;
    this.target = target;
    this.serverNames = Array.isArray(serverNames)
      ? serverNames.join(",")
      : serverNames;
...
class Inbound extends XrayCommonClass {
  ...
  get host() {
    if (this.isTcp) {
      ...
      return this.stream.ws.host?.length > 0
        ? this.stream.ws.host
        : this.getHeader(this.stream.ws, "host");
    } else if (this.isHttpupgrade) {
      return this.stream.httpupgrade.host?.length > 0
        ? this.stream.httpupgrade.host
        : this.getHeader(this.stream.httpupgrade, "host");
    } else if (this.isXHTTP) {
      return this.stream.xhttp.host?.length > 0
        ? this.stream.xhttp.host
        : this.getHeader(this.stream.xhttp, "host");
    }
    return null;
  }
  ...
  genVLESSLink(
    ...
    switch (type) {
      ...
      case "ws":
        ...
        params.set(
          "host",
          ws.host?.length > 0 ? ws.host : this.getHeader(ws, "host"),
        );
        break;
      case "grpc":
  ...
  genLink(
    ...
    switch (this.protocol) {
      ...
      case Protocols.SHADOWSOCKS:
        return this.genSSLink(
          address,
          port,
          forceTls,
          remark,
          this.isSSMultiUser ? client.password : "",
        );
      case Protocols.TROJAN:
        return this.genTrojanLink(
          address,
          port,
          forceTls,
          remark,
    ...
    if (this.clients) {
      ...
      if (this.protocol == Protocols.WIREGUARD) {
        ...
        this.settings.peers.forEach((p, index) => {
          links.push(
            this.getWireguardLink(
              addr,
              this.port,
              remark + remarkModel.charAt(0) + (index + 1),
              index,
            ),
          );
        });
...
Inbound.WireguardSettings = class extends XrayCommonClass {
  ...
  addPeer() {
    this.peers.push(
      new Inbound.WireguardSettings.Peer(null, null, "", [
        "10.0.0." + (this.peers.length + 2),
      ]),
    );
  }
...
Inbound.WireguardSettings.Peer = class extends XrayCommonClass {
  constructor(
    ...
    this.publicKey = publicKey;
    if (!this.publicKey) {
      [this.publicKey, this.privateKey] = Object.values(
