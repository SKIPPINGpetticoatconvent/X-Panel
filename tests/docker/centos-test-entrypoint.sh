...
echo "=== CentOS测试环境启动 ==="
echo "操作系统类型: CentOS 7"
echo "Go版本: $(go version)"
echo "当前用户: $(whoami)"
echo "工作目录: $(pwd)"

# 设置测试环境变量
export TEST_OS_TYPE=centos
...
# 显示包管理器信息
echo "=== 包管理器信息 ==="
command -v yum >/dev/null 2>&1 && echo "YUM包管理器可用"
command -v dnf >/dev/null 2>&1 && echo "DNF包管理器可用"
command -v rpm >/dev/null 2>&1 && echo "RPM包管理器可用"
